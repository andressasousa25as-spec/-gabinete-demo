...
import { coordConfiavel, MACAPA_CENTRO, AMAPA_BBOX, COORDS_BAIRROS, LISTA_BAIRROS } from '../lib/bairros';
...
export default function DashboardCandidato({ perfil, ehMaster }) {
  ...
  const cadastrarLider = async () => {
    ...
    if (!error) {
      ...
      await registrarLog('Cadastrou liderança', `Nome: ${novaLider.nome} | Bairro: ${novaLider.bairro || '-'}`); alert('✅ Liderança salva!'); fetchAll(); setNovaLider({ nome: '', telefone: '', bairro: '', demanda: '', endereco: '', zona_eleitoral: '', secao_eleitoral: '' }); setShowLider(false);
    }
    else alert('Erro: ' + error.message);
    setLoading(false);
  };
      ...
      {showEleitor && (
        <div style={estiloModal} onClick={e => e.target === e.currentTarget && setShowEleitor(false)}>
          <div style={estiloCard}>
            ...
            <select style={estiloInput} value={novoEleitor.bairro} onChange={e => setNovoEleitor({ ...novoEleitor, bairro: e.target.value })}>
              <option value="">Selecione o bairro...</option>
              {BAIRROS_AMAPA.map(b => <option key={b} value={b}>{b}</option>)}
            </select>
