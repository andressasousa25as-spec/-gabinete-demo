...
export default function GestaoMidias({ onVoltar }) {
  ...
  const fetchAll = async () => {
    ...
    setMidias(mData || []);
    const { data: eData } = await supabase.from('eleitores').select('id, nome, telefone, bairro, lideranca_id').not('telefone', 'is', null).eq('opt_out', false);
  ...
  const upload = async () => {
    ...
    try {
      ...
      const { error: dbError } = await supabase.from('midias').insert({ titulo, descricao, tipo: detectarTipo(arquivo), arquivo_url: urlData.publicUrl, arquivo_path: path, link_rastreavel: urlData.publicUrl });
  ...
  const abrirDisparo = (midia) => { setMidiaDisparo(midia); setSelecionados(eleitores.map(e => e.id)); setListaLinks([]); };
  const toggleSelecionado = (id) => setSelecionados(prev => prev.includes(id) ? prev.filter(i => i !== id) : [...prev, id]);
  const selecionarTodos = () => { if (selecionados.length === eleitores.length) setSelecionados([]); else setSelecionados(eleitores.map(e => e.id)); };
  ...
  const filtrarPorLideranca = (liderancaId) => {
    ...
    setSelecionados(eleitores.filter(e => e.lideranca_id === liderancaId).map(e => e.id));
  };

  const copiarLink = (midia) => { navigator.clipboard.writeText(midia.arquivo_url); setLinkCopiado(midia.id); setTimeout(() => setLinkCopiado(null), 2000); };
      ...
      <div style={{ background: 'white', borderRadius: '20px', padding: '24px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
        ...
        {loading ? <p style={{ color: '#6b7280', textAlign: 'center' }}>⏳ Carregando...</p> :
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {midias.map(m => (
              <div key={m.id} style={{ borderRadius: '12px', padding: '16px', border: '1px solid #334155', background: '#0f172a', color: '#f1f5f9' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px' }}>
                  <div style={{ flex: 1 }}>
                    <p style={{ fontWeight: 'bold', fontSize: '15px', color: '#f1f5f9' }}>{icone(m.tipo)} {m.titulo}</p>
                    {m.descricao && <p style={{ color: '#94a3b8', fontSize: '13px', marginTop: '4px' }}>{m.descricao}</p>}
                    <p style={{ color: '#64748b', fontSize: '12px', marginTop: '4px' }}>{m.tipo?.toUpperCase()} — {new Date(m.created_at).toLocaleString('pt-BR')}</p>
                  </div>
                ...
                <div style={{ display: 'flex', gap: '8px', marginTop: '12px', flexWrap: 'wrap' }}>
                  ...
                  <button onClick={() => abrirDisparo(m)} style={{ padding: '7px 14px', background: '#25D366', color: 'white', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: 'bold', cursor: 'pointer' }}>📲 Disparar WhatsApp</button>
        ...
        <div style={estiloModal} onClick={e => e.target === e.currentTarget && setMidiaDisparo(null)}>
          <div style={estiloCardModal}>
            ...
            <div style={{ marginBottom: 12 }}>
              <select value={filtroLideranca} onChange={e => filtrarPorLideranca(e.target.value)} style={{ width: '100%', padding: '10px', borderRadius: 8, border: '1px solid #334155', background: '#1e293b', color: '#f1f5f9', fontSize: 14 }}>
        ...
        <div style={estiloModal} onClick={e => e.target === e.currentTarget && setListaLinks([])}>
          <div style={estiloCardModal}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '16px' }}>
              {listaLinks.map((l, i) => (
                <a key={i} href={l.url} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', background: '#f0fdf4', borderRadius: '10px', border: '1px solid #86efac', textDecoration: 'none' }}>
                  <div>
