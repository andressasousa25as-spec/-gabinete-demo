...
export default function Comunicado({ eleitores = [], liderancas = [], reunioes = [], onEnviar, onClose }) {
  ...
  const sel = { width: '100%', padding: 10, borderRadius: 8, border: '1px solid #334155', background: '#1e293b', color: '#e2e8f0', fontSize: 14, marginBottom: 12 };

  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
      onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={card}>
        <h2 style={{ color: '#f1f5f9', margin: '0 0 4px', fontSize: 18, fontWeight: 800 }}>📣 Comunicado por liderança</h2>
        <p style={{ color: '#94a3b8', fontSize: 13, margin: '0 0 16px' }}>Avise uma liderança e os apoiadores ligados a ela. Cada pessoa recebe um link de WhatsApp pra você enviar.</p>

        {links.length === 0 ? (
          <>
            <label style={{ color: '#cbd5e1', fontSize: 13, fontWeight: 700 }}>Liderança</label>
            ...
            <label style={{ color: '#cbd5e1', fontSize: 13, fontWeight: 700 }}>Reunião (opcional — preenche a mensagem)</label>
            ...
            <label style={{ color: '#cbd5e1', fontSize: 13, fontWeight: 700 }}>Mensagem {reuniao ? '(complemento opcional)' : ''}</label>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <div style={{ maxHeight: 200, overflowY: 'auto', border: '1px solid #1f2937', borderRadius: 10, padding: 8, marginTop: 6 }}>
                  {destinatarios.length === 0 ? <p style={{ color: '#64748b', textAlign: 'center', padding: 12, fontSize: 13 }}>Sem destinatários com telefone.</p> :
                    destinatarios.map(p => (
                      <label key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 6px', borderBottom: '1px solid #1f2937', cursor: 'pointer' }}>
                        <input type="checkbox" checked={!desmarcados.includes(p.id)} onChange={() => toggle(p.id)} />
                        <span style={{ color: '#e2e8f0', fontSize: 13 }}>{p.nome} <span style={{ color: '#64748b' }}>· {p.tipo === 'lideranca' ? 'liderança' : 'apoiador'}</span></span>
                      </label>
                    ))}
                </div>
              </div>
          ...
          <div style={{ marginBottom: 8 }}>
            <p style={{ color: '#cbd5e1', fontSize: 13, fontWeight: 700, marginBottom: 8 }}>✅ {links.length} link{links.length !== 1 ? 's' : ''} — clique pra enviar pelo WhatsApp:</p>
            ...
            <button onClick={() => setLinks([])} style={{ width: '100%', padding: 11, background: '#1e293b', color: '#cbd5e1', border: 'none', borderRadius: 10, fontWeight: 600, fontSize: 14, cursor: 'pointer', marginTop: 10 }}>← Voltar</button>
          </div>
        )}
