    ...
    <>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 14, marginBottom: 24 }}>
        <div style={{ background: 'var(--surface-2)', borderRadius: 12, border: '1px solid var(--border)', padding: '16px 18px' }}>
          <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', marginBottom: 6 }}>Votos 2024 ({sel.municipio})</div>
          <div style={{ fontSize: 22, fontWeight: 700, color: 'var(--text-muted)' }}>{sel.votos.toLocaleString('pt-BR')}</div>
        </div>
        ...
        <div style={{ background: '#f0fdf4', borderRadius: 12, border: '1px solid #bbf7d0', padding: '16px 18px' }}>
          ...
          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>faltam {Math.max(0, meta - sel.votos).toLocaleString('pt-BR')} votos</div>
        </div>
      ...
      {loading && <div style={{ ...cardStyle, color: 'var(--text-muted)', textAlign: 'center' }}>Carregando dados por zona...</div>}
      {semDados && !loading && <div style={{ ...cardStyle, color: 'var(--text-muted)', textAlign: 'center' }}>Dados por zona não disponíveis para este candidato.</div>}
                ...
                {zonasComMeta.map((z, i) => {
                  ...
                  return (
                    <tr key={i} style={{ background: i % 2 === 0 ? 'var(--surface)' : 'var(--surface-2)' }}>
                      ...
                      <td style={{ ...tdStyle, color: 'var(--text-muted)' }}>{z.votos.toLocaleString('pt-BR')}</td>
      ...
      {!loading && !semDados && aba === 'secoes' && (
        <div style={cardStyle}>
          ...
          <p style={{ margin: '0 0 16px', fontSize: 12, color: 'var(--text-muted)' }}>{secoes?.length} seções em {sel.municipio}</p>
        ...
        <div>
          ...
          <div style={cardStyle}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 8 }}>
              <span style={{ color: 'var(--text-muted)' }}>Base 2024: <strong>{sel.votos.toLocaleString('pt-BR')}</strong></span>
  ...
  return (
    <div style={{ padding: '0 0 40px', fontFamily: 'Inter, sans-serif', color: 'var(--text)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
        ...
        <div>
          <h2 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: 'var(--text)' }}>Cenário Vereador 2024 → Deputado 2026</h2>
          <p style={{ margin: 0, fontSize: 13, color: 'var(--text-muted)' }}>Dados reais TSE 2024 — Macapá e Santana</p>
        </div>
      </div>
      ...
      <div style={cardStyle}>
        ...
        {sel && (
          <div style={{ background: '#eff6ff', border: '1.5px solid #93c5fd', borderRadius: 10, padding: '12px 16px', marginBottom: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              <p style={{ margin: 0, fontSize: 12, color: 'var(--text-muted)' }}>Vereador {sel.municipio} · {sel.partido} · {sel.votos.toLocaleString('pt-BR')} votos em 2024 · {sel.situacao}</p>
            </div>
        ...
        {busca && !sel && (
          <div style={{ maxHeight: 260, overflowY: 'auto', border: '1px solid var(--border)', borderRadius: 8 }}>
            {filtrados.length === 0 ? (
              <p style={{ padding: 16, color: 'var(--text-muted)', textAlign: 'center' }}>Nenhum vereador encontrado.</p>
            ) : filtrados.map((c, i) => (
              <div key={i} onClick={() => selecionar(c)}
                ...
                <div>
                  ...
                  <p style={{ margin: 0, fontSize: 12, color: 'var(--text-muted)' }}>Vereador {c.municipio} · {c.partido}{c.eleito ? ' · ELEITO' : ''}</p>
                </div>
