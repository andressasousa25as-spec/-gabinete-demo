...
export default function Dashboard({ candidato, perfil, onLogout }) {
  ...
  const [novoEleitor, setNovoEleitor] = useState({ nome: '', telefone: '', bairro: '', logradouro: '', zona_eleitoral: '', secao_eleitoral: '', cidade: 'Macap�' });
  ...
  const cadastrarEleitor = async () => {
    ...
    if (!novoEleitor.nome || !novoEleitor.telefone) return alert('Nome e telefone s�o obrigat�rios.');
    setLoading(true);
    const { error } = await supabase.from('eleitores').insert([{ ...novoEleitor, consentimento_lgpd: true }]);
    if (!error) { alert('Eleitor cadastrado!'); fetchAll(); setShowEleitor(false); setTermoAceito(false); setNovoEleitor({ nome: '', telefone: '', bairro: '', logradouro: '', zona_eleitoral: '', secao_eleitoral: '', cidade: 'Macap�' }); }
    else alert('Erro: ' + error.message);
    setLoading(false);
  };

  const cadastrarLider = async () => {
    if (!novaLider.nome) return alert('Nome � obrigat�rio.');
    setLoading(true);
    const { error } = await supabase.from('liderancas').insert([novaLider]);
    if (!error) { alert('Lideran�a salva!'); fetchAll(); setShowLider(false); setNovaLider({ nome: '', telefone: '', bairro: '', demanda: '' }); }
    else alert('Erro: ' + error.message);
    setLoading(false);
  };

  const cadastrarReuniao = async () => {
    if (!novaReuniao.titulo || !novaReuniao.data) return alert('T�tulo e data s�o obrigat�rios.');
    setLoading(true);
    const { error } = await supabase.from('reunioes').insert([novaReuniao]);
    if (!error) { alert('Reuni�o agendada!'); fetchAll(); setShowReuniao(false); setNovaReuniao({ titulo: '', data: '', local: '', endereco: '' }); }
    else alert('Erro: ' + error.message);
    setLoading(false);
  };

  const cadastrarAnotacao = async () => {
    if (!novaAnotacao.titulo) return alert('T�tulo � obrigat�rio.');
    setLoading(true);
    const { error } = await supabase.from('anotacoes').insert([novaAnotacao]);
    if (!error) { alert('Anota��o salva!'); fetchAll(); setShowAnotacao(false); setNovaAnotacao({ titulo: '', conteudo: '' }); }
    else alert('Erro: ' + error.message);
    setLoading(false);
  };
  ...
  const abasCandidato = [
    ...
    { id: 'liderancas', label: '?? Lideran�as' },
    { id: 'reunioes', label: '?? Reuni�es' },
    { id: 'mapa', label: '??? Mapa' },
    { id: 'anotacoes', label: '?? Anota��es' },
    { id: 'midias', label: '?? M�dias' },
  ...
  if (aba === 'relatorios') return (
    <div style={{ minHeight: '100vh', background: '#0f172a', color: 'white', padding: 24 }}>
      ...
      <h2 style={{ color: 'white', marginBottom: 20 }}>??? Relat�rios</h2>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
        {[
          ...
          { label: 'Lideran�as', dados: liderancas.map(l => ({ nome: l.nome, telefone: l.telefone || '-', bairro: l.bairro || '-', demanda: l.demanda || '-' })), colunas: ['nome','telefone','bairro','demanda'] },
          { label: 'Reuni�es', dados: reunioes.map(r => ({ titulo: r.titulo, data: r.data ? new Date(r.data).toLocaleString('pt-BR') : '-', local: r.local || '-' })), colunas: ['titulo','data','local'] },
          { label: 'Anota��es', dados: anotacoes.map(a => ({ titulo: a.titulo, conteudo: a.conteudo || '-' })), colunas: ['titulo','conteudo'] },
        ].map((rel, i) => (
    ...
    <div style={{ minHeight: '100vh', background: '#0f172a', color: 'white' }}>
      ...
      <main style={{ padding: 24, maxWidth: 1280, margin: '0 auto' }}>
        ...
        {(aba === 'inicio' || aba === 'eleitores') && (
          <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap' }}>
            ...
            {perfil === 'candidato' && <button onClick={() => setShowLider(true)} style={{ background: '#7c3aed', color: 'white', border: 'none', padding: '12px 20px', borderRadius: 10, cursor: 'pointer', fontWeight: 700, fontSize: 14 }}>+ Lideran�a</button>}
            <button onClick={() => setShowReuniao(true)} style={{ background: '#d97706', color: 'white', border: 'none', padding: '12px 20px', borderRadius: 10, cursor: 'pointer', fontWeight: 700, fontSize: 14 }}>+ Reuni�o</button>
          </div>
        )}
        ...
        {aba === 'inicio' && (
          <div>
            <div style={{ background: 'linear-gradient(135deg, #1e3a5f, #1e40af)', borderRadius: 16, padding: 24, marginBottom: 24, textAlign: 'center' }}>
              ...
              <p style={{ color: '#93c5fd', margin: '8px 0 0' }}>Eleitores: {eleitores.length} � Lideran�as: {liderancas.length}</p>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
              {card('Total de Eleitores', eleitores.length, `Meta: ${metaEleitores.toLocaleString('pt-BR')}`, '#60a5fa')}
              {card('Lideran�as Ativas', liderancas.length, 'Meta: 200', '#f59e0b')}
              {card('Reuni�es', reunioes.length, 'agendadas e realizadas', '#34d399')}
              {card('Anota��es', anotacoes.length, 'registradas', '#a78bfa')}
            </div>
            <h3 style={{ fontSize: 16, color: '#94a3b8', marginBottom: 12 }}>?? Pr�ximas Reuni�es</h3>
              ...
              {eleitores.map(e => (
                <div key={e.id} style={{ background: '#1e293b', borderRadius: 10, padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
                  <div>
                    ...
                    <p style={{ color: '#94a3b8', fontSize: 13, margin: '2px 0 0' }}>?? {e.bairro || '-'} � Zona {e.zona_eleitoral || '-'} Se��o {e.secao_eleitoral || '-'}</p>
                  </div>
        ...
        {aba === 'liderancas' && perfil === 'candidato' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h2 style={{ fontSize: 22, margin: 0, color: 'white' }}>?? Lideran�as ({liderancas.length})</h2>
              <button onClick={() => setShowLider(true)} style={{ background: '#7c3aed', color: 'white', border: 'none', padding: '10px 18px', borderRadius: 10, cursor: 'pointer', fontWeight: 700 }}>+ Lideran�a</button>
            </div>
        ...
        {aba === 'reunioes' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h2 style={{ fontSize: 22, margin: 0, color: 'white' }}>?? Reuni�es ({reunioes.length})</h2>
              <button onClick={() => setShowReuniao(true)} style={{ background: '#d97706', color: 'white', border: 'none', padding: '10px 18px', borderRadius: 10, cursor: 'pointer', fontWeight: 700 }}>+ Reuni�o</button>
            </div>
        ...
        {aba === 'anotacoes' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h2 style={{ fontSize: 22, margin: 0, color: 'white' }}>?? Anota��es ({anotacoes.length})</h2>
              <button onClick={() => setShowAnotacao(true)} style={{ background: '#1e40af', color: 'white', border: 'none', padding: '10px 18px', borderRadius: 10, cursor: 'pointer', fontWeight: 700 }}>+ Anota��o</button>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
              {anotacoes.map(a => (
      ...
      {showEleitor && (
        <div style={estiloModal} onClick={e => e.target === e.currentTarget && setShowEleitor(false)}>
          <div style={estiloCard}>
            ...
            <input style={estiloInput} placeholder="Endere�o" value={novoEleitor.logradouro} onChange={e => setNovoEleitor({...novoEleitor, logradouro: e.target.value})} />
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <input style={{...estiloInput, flex: 1}} placeholder="Se��o" value={novoEleitor.secao_eleitoral} onChange={e => setNovoEleitor({...novoEleitor, secao_eleitoral: e.target.value})} />
            </div>
      ...
      {showLider && (
        <div style={estiloModal} onClick={e => e.target === e.currentTarget && setShowLider(false)}>
          <div style={estiloCard}>
            <h2 style={{ color: '#7c3aed', marginBottom: 20 }}>? Cadastrar Lideran�a</h2>
            ...
            <button onClick={cadastrarLider} disabled={loading} style={estiloBotao('#7c3aed')}>{loading ? 'Salvando...' : '? Cadastrar Lideran�a'}</button>
      ...
      {showReuniao && (
        <div style={estiloModal} onClick={e => e.target === e.currentTarget && setShowReuniao(false)}>
          <div style={estiloCard}>
            <h2 style={{ color: '#d97706', marginBottom: 20 }}>? Agendar Reuni�o</h2>
            <input style={estiloInput} placeholder="T�tulo *" value={novaReuniao.titulo} onChange={e => setNovaReuniao({...novaReuniao, titulo: e.target.value})} />
            ...
            <button onClick={cadastrarReuniao} disabled={loading} style={estiloBotao('#d97706')}>{loading ? 'Salvando...' : '? Agendar Reuni�o'}</button>
      ...
      {showAnotacao && (
        <div style={estiloModal} onClick={e => e.target === e.currentTarget && setShowAnotacao(false)}>
          <div style={estiloCard}>
            <h2 style={{ color: '#1e40af', marginBottom: 20 }}>?? Nova Anota��o</h2>
            <input style={estiloInput} placeholder="T�tulo *" value={novaAnotacao.titulo} onChange={e => setNovaAnotacao({...novaAnotacao, titulo: e.target.value})} />
            <textarea style={{...estiloInput, resize: 'vertical'}} placeholder="Conte�do" rows={4} value={novaAnotacao.conteudo} onChange={e => setNovaAnotacao({...novaAnotacao, conteudo: e.target.value})} />
            <button onClick={cadastrarAnotacao} disabled={loading} style={estiloBotao('#1e40af')}>{loading ? 'Salvando...' : '? Salvar Anota��o'}</button>
