  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0f172a', color: 'white' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          >
            {foto
              ...
              : <span style={{ fontSize: 14, color: '#64748b' }}>Foto</span>
            }
          </div>
          {perfil === 'candidato' && (
            ...
            {editandoNome && perfil === 'candidato' ? (
              <div style={{ display: 'flex', gap: 6, marginTop: 4 }}>
                ...
                <button onClick={() => { setNomeAtual(nomeEdit); setEditandoNome(false); }}
                  style={{ background: '#16a34a', color: 'white', border: 'none', borderRadius: 6, padding: '4px 8px', cursor: 'pointer', fontSize: 12 }}>OK</button>
                <button onClick={() => setEditandoNome(false)}
                  style={{ background: '#ef4444', color: 'white', border: 'none', borderRadius: 6, padding: '4px 8px', cursor: 'pointer', fontSize: 12 }}>X</button>
              </div>
            ) : (
              <p style={{ color: '#60a5fa', fontSize: 14, margin: '2px 0 0', cursor: perfil === 'candidato' ? 'pointer' : 'default' }}
                ...
                {nomeAtual}
                {perfil === 'candidato' && <span style={{ fontSize: 11, color: '#64748b', marginLeft: 6 }}>editar</span>}
              </p>
            )}
      ...
      <main style={{ padding: 24, maxWidth: 1280, margin: '0 auto' }}>
        ...
        {aba === 'inicio' && (
          <div>
            <h2 style={{ fontSize: 22, marginBottom: 20, color: 'white' }}>Painel Geral</h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 24 }}>
              {card('Total de Eleitores', METRICAS.totalEleitores, `Meta: ${METRICAS.metaEleitores.toLocaleString()}`, '#60a5fa')}
              {card('Liderancas Ativas', METRICAS.totalLiderancas, `Meta: ${METRICAS.metaLiderancas}`, '#f59e0b')}
              {card('Reunioes Realizadas', METRICAS.reunioesRealizadas, `${METRICAS.reunioesAgendadas} agendadas`, '#34d399')}
              {card('Bairros Cobertos', METRICAS.bairrosCobertos, 'em Macapa', '#a78bfa')}
            </div>
            <h3 style={{ fontSize: 16, color: '#94a3b8', marginBottom: 12 }}>Proximas Reunioes</h3>
              ...
              {eleitores.map(e => (
                <div key={e.id} style={{ background: '#1e293b', borderRadius: 10, padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
                  <div>
                    ...
                    <p style={{ color: '#94a3b8', fontSize: 13, margin: '2px 0 0' }}>{e.bairro} &mdash; Zona {e.zona} Secao {e.secao}</p>
                  </div>
        ...
        {aba === 'reunioes' && (
          <div>
            <h2 style={{ fontSize: 22, marginBottom: 20, color: 'white' }}>Agenda de Reunioes</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {REUNIOES.map(r => (
                <div key={r.id} style={{ background: '#1e293b', borderRadius: 10, padding: '14px 18px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
                  <div>
                    <p style={{ fontWeight: 600, margin: 0 }}>{r.titulo}</p>
                    <p style={{ color: '#94a3b8', fontSize: 13, margin: '4px 0 0' }}>{r.local} &mdash; {new Date(r.data).toLocaleString('pt-BR')}</p>
                  </div>
