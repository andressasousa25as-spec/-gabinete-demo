...
insert into public.comparativo_internos (nome, eh_nosso, votos, cargo_ultima, abrangencia, confirmado, risco, observacao, ordem) values
  ...
  ('Divino Rocha', false, 0, 'Prefeito Ferreira Gomes 2020 (cand.)', 'Município', false, 'MEDIO', 'Candidato a prefeito de Ferreira Gomes em 2020 (Progressistas). Base local organizada, mas município pequeno (~7 mil eleitores) → peso estadual limitado. Votos a confirmar no TSE.', 16);
```
