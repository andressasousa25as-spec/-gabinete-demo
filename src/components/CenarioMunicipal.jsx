...
// Deputados estaduais disponíveis no TSE 2022
const DEPUTADOS = CANDIDATOS_TSE.filter(c => c.cargo === 'DEPUTADO ESTADUAL')
  .sort((a, b) => (b.total || 0) - (a.total || 0))

// Normaliza busca de votos por município (encoding latin1 quebrado nas chaves)
function getVotosMun(candidato, nomeMun) {
  if (!candidato?.municipios) return 0
  const chave = Object.keys(candidato.municipios).find(k =>
    k.toUpperCase().includes(nomeMun.toUpperCase().substring(0, 5))
  )
...
function corPartido(p) { return COR_PARTIDO[p] || COR_PARTIDO.default }
...
export default function CenarioMunicipal({ onVoltar }) {
  ...
  const aliados = useMemo(() => eleitos.filter(v => v.partido === PARTIDO_CAND), [eleitos, PARTIDO_CAND])
  const outros = useMemo(() => eleitos.filter(v => v.partido !== PARTIDO_CAND), [eleitos, PARTIDO_CAND])

  const votosCandMun = municipio === 'macapa'
    ? getVotosMun(candidato, 'MACAP')
    : getVotosMun(candidato, 'SANTANA')
  ...
  return (
    <div style={{ padding: '1rem', maxWidth: 820, margin: '0 auto', fontFamily: 'sans-serif' }}>
      <button onClick={onVoltar} style={{ background: '#1e3a8a', color: 'white', border: 'none', borderRadius: 8, padding: '8px 16px', cursor: 'pointer', fontWeight: 600, fontSize: 13, marginBottom: 16 }}>← Voltar</button>
      ...
      <div style={{ background: '#eff6ff', border: '2px solid #1d4ed8', borderRadius: 12, padding: '1rem', marginBottom: '1.2rem' }}>
        <label style={{ fontWeight: 700, color: '#1e3a8a', fontSize: '0.9rem', display: 'block', marginBottom: 8 }}>
          🎯 Selecione o candidato para análise
        </label>
        <select
          value={candidatoIdx}
          onChange={e => { setCandidatoIdx(Number(e.target.value)); setFiltroPartido('TODOS'); setBusca('') }}
        ...
        {candidato && (
          <div style={{ display: 'flex', gap: 16, marginTop: 10, flexWrap: 'wrap' }}>
            <span style={{ fontSize: '0.82rem', color: '#475569' }}>
              <strong>Total AP 2022:</strong> {(candidato.total || 0).toLocaleString('pt-BR')} votos
            </span>
            <span style={{ fontSize: '0.82rem', color: '#475569' }}>
              <strong>Macapá:</strong> {getVotosMun(candidato, 'MACAP').toLocaleString('pt-BR')}
            </span>
            <span style={{ fontSize: '0.82rem', color: '#475569' }}>
      ...
      <div style={{ display: 'flex', gap: 8, justifyContent: 'center', marginBottom: '1rem' }}>
        {['macapa', 'santana'].map(m => (
          <button key={m} onClick={() => { setMunicipio(m); setFiltroPartido('TODOS'); setBusca('') }}
            style={{ padding: '8px 24px', borderRadius: 999, border: 'none', cursor: 'pointer', fontWeight: 600, background: municipio === m ? '#1e3a8a' : '#e2e8f0', color: municipio === m ? '#fff' : '#475569', fontSize: '0.9rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 10, marginBottom: '1rem' }}>
        {[
          { label: 'Cadeiras total', valor: totalEleitos, cor: '#1e3a8a' },
          { label: `Cadeiras ${PARTIDO_CAND || '—'}`, valor: aliados.length, cor: '#1d4ed8', destaque: true },
          { label: 'Quociente eleitoral', valor: qe.toLocaleString('pt-BR'), cor: '#0891b2' },
          { label: 'Votos candidato', valor: votosCandMun.toLocaleString('pt-BR'), cor: '#059669' },
        ].map((c, i) => (
          <div key={i} style={{ background: c.destaque ? '#eff6ff' : '#f8fafc', border: c.destaque ? '2px solid #1d4ed8' : '1px solid #e2e8f0', borderRadius: 12, padding: '12px 10px', textAlign: 'center' }}>
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: '1rem', borderBottom: '2px solid #e2e8f0' }}>
        {[
          { id: 'aliados', label: `🤝 Aliados ${PARTIDO_CAND}` },
      ...
      {aba === 'aliados' && (
        <div>
          <div style={{ background: '#f8fafc', borderRadius: 12, padding: '1rem', marginBottom: '1rem' }}>
            <h3 style={{ fontWeight: 700, color: '#1e293b', marginBottom: 10, fontSize: '0.9rem' }}>Distribuição de cadeiras por partido</h3>
            {distPartidos.map(([partido, qtd]) => (
              <div key={partido} style={{ marginBottom: 8 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', marginBottom: 3 }}>
                  <span style={{ fontWeight: partido === PARTIDO_CAND ? 800 : 600, color: partido === PARTIDO_CAND ? corPartido(partido) : '#374151' }}>
                    {partido === PARTIDO_CAND ? '⭐ ' : ''}{partido}
                  </span>
                  <span style={{ color: '#6b7280' }}>{qtd} cadeira{qtd > 1 ? 's' : ''}</span>
                </div>
                <div style={{ background: '#e2e8f0', borderRadius: 4, height: 7 }}>
                  <div style={{ width: `${(qtd / totalEleitos) * 100}%`, height: 7, borderRadius: 4, background: corPartido(partido) }} />
                </div>
              </div>
            ))}
          </div>
          ...
          <h3 style={{ fontWeight: 700, color: '#1e293b', marginBottom: 8, fontSize: '0.9rem' }}>
            🤝 Vereadores {PARTIDO_CAND} eleitos — potenciais aliados 2026
          </h3>
          {aliados.length === 0 ? (
            <div style={{ background: '#fef9c3', borderRadius: 10, padding: '1rem', color: '#854d0e', fontSize: '0.88rem' }}>
              ⚠️ Nenhum vereador do {PARTIDO_CAND} eleito em {dados.municipio}. Considere aliados de federação ou coligação.
            </div>
          ) : aliados.map((v, i) => (
            <div key={v.sq} style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 10, padding: '10px 14px', marginBottom: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <div style={{ fontWeight: 700, color: '#1e3a8a', fontSize: '0.9rem' }}>#{i + 1} {v.nomeUrna}</div>
                <div style={{ fontSize: '0.75rem', color: '#64748b', marginTop: 2 }}>Nº {v.numero} · {v.partido}</div>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontWeight: 800, color: '#1d4ed8', fontSize: '1rem' }}>{v.votos.toLocaleString('pt-BR')}</div>
                <div style={{ fontSize: '0.7rem', color: '#94a3b8' }}>votos</div>
              </div>
            </div>
          ))}

          <h3 style={{ fontWeight: 700, color: '#1e293b', margin: '1rem 0 8px', fontSize: '0.9rem' }}>🔎 Demais vereadores eleitos</h3>
          {outros.map(v => (
            <div key={v.sq} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, padding: '10px 14px', marginBottom: 6, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <span style={{ background: corPartido(v.partido) + '22', color: corPartido(v.partido), padding: '1px 7px', borderRadius: 4, fontWeight: 700, fontSize: '0.72rem' }}>{v.partido}</span>
              </div>
              <div style={{ fontWeight: 700, color: '#374151', fontSize: '0.95rem' }}>{v.votos.toLocaleString('pt-BR')} <span style={{ fontSize: '0.68rem', color: '#94a3b8' }}>vts</span></div>
            </div>
          ))}
        </div>
      )}
      ...
      {aba === 'ranking' && (
        <div>
          ...
          {candidatosFiltrados.map((v, i) => (
            <div key={v.sq} style={{ background: v.eleito ? '#f0fdf4' : '#f8fafc', border: v.eleito ? '1px solid #bbf7d0' : '1px solid #e2e8f0', borderLeft: v.partido === PARTIDO_CAND ? `4px solid ${corPartido(PARTIDO_CAND)}` : undefined, borderRadius: 8, padding: '9px 12px', marginBottom: 5, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', gap: 5, marginTop: 2 }}>
                    <span style={{ background: corPartido(v.partido) + '22', color: corPartido(v.partido), padding: '1px 5px', borderRadius: 3, fontWeight: 700, fontSize: '0.7rem' }}>{v.partido}</span>
                    <span style={{ fontSize: '0.7rem', color: v.eleito ? '#16a34a' : '#94a3b8', fontWeight: 600 }}>{v.eleito ? '✓ Eleito' : 'Não eleito'}</span>
                  </div>
                </div>
              </div>
        ...
        <div>
          <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 12, padding: '1rem', marginBottom: '1rem' }}>
            <h3 style={{ fontWeight: 700, color: '#1e3a8a', marginBottom: 8, fontSize: '0.9rem' }}>
              📊 {candidato?.nome || '—'} (Dep. Estadual 2022) × Câmara Municipal 2024
            </h3>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <div style={{ background: '#fff', borderRadius: 8, padding: '10px', textAlign: 'center' }}>
                <div style={{ fontSize: '1.3rem', fontWeight: 800, color: '#1d4ed8' }}>
                  {votosCandMun > 0 ? votosCandMun.toLocaleString('pt-BR') : '—'}
                </div>
          ...
          <h3 style={{ fontWeight: 700, color: '#1e293b', marginBottom: 10, fontSize: '0.88rem' }}>
            🎯 Força relativa vs vereadores eleitos
          </h3>
          {eleitos.map(v => {
            const pct = votosCandMun > 0 ? Math.round((v.votos / votosCandMun) * 100) : 0
            const aliado = v.partido === PARTIDO_CAND
            return (
              <div key={v.sq} style={{ marginBottom: 10 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', marginBottom: 3 }}>
                  <span style={{ fontWeight: aliado ? 800 : 600, color: aliado ? corPartido(PARTIDO_CAND) : '#374151' }}>
                    {aliado ? '⭐ ' : ''}{v.nomeUrna}
                    <span style={{ marginLeft: 6, fontSize: '0.68rem', background: corPartido(v.partido) + '22', color: corPartido(v.partido), padding: '1px 5px', borderRadius: 3, fontWeight: 700 }}>{v.partido}</span>
                  </span>
                  <span style={{ color: '#6b7280' }}>{v.votos.toLocaleString('pt-BR')} vts</span>
                </div>
                <div style={{ background: '#e2e8f0', borderRadius: 4, height: 6 }}>
                  <div style={{ width: `${Math.min(pct, 100)}%`, height: 6, borderRadius: 4, background: aliado ? corPartido(PARTIDO_CAND) : '#94a3b8' }} />
                </div>
                {votosCandMun > 0 && <div style={{ fontSize: '0.68rem', color: '#94a3b8', marginTop: 1 }}>{pct}% dos votos do candidato em {dados.municipio}</div>}
              </div>
            )
          })}
