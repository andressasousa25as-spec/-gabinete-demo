      ...
      {midiaDisparo && (
        <div style={estiloModal} onClick={e => e.target === e.currentTarget && setMidiaDisparo(null)}>
          <div style={estiloCardModal}>
            ...
            <p style={{ color: '#6b7280', fontSize: '13px', marginBottom: '12px' }}>Escolha o público-alvo, selecione e clique em Gerar Links.</p>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
              {[{k:'apoiadores',l:'👤 Apoiadores'},{k:'liderancas',l:'⭐ Lideranças'}].map(g => (
                <button key={g.k} type="button" onClick={() => trocarGrupo(g.k)}
                  style={{ flex: 1, padding: '9px', borderRadius: 8, border: grupoAlvo===g.k?'2px solid #1e40af':'1px solid #cbd5e1', background: grupoAlvo===g.k?'#eff6ff':'white', color: grupoAlvo===g.k?'#1e40af':'#64748b', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
                  {g.l}
                </button>
              ))}
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
              <span style={{ fontWeight: 'bold', fontSize: '14px' }}>{selecionados.length} de {listaDoGrupo(grupoAlvo).length} selecionados</span>
